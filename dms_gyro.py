...
class DMSGyro:
    def __init__(self):
        ...
        try:
            ...
            print("🏎️  Giroscopio MPU6050 (ID 0x70) Inizializzato")
        except Exception as e:
            print(f"❌ Errore Init Gyro: {e}")
            self.active = False
    ...
    def get_status(self):
        ...
        try:
            # Lettura Hardware (m/s^2)
            acc_x, acc_y, acc_z = self.sensor.acceleration
            # Calcolo G-Force Totale (Vettore 3D)
            # Formula: radice(x^2 + y^2 + z^2) / 9.81
